...
static void _on_resuming (G_GNUC_UNUSED DBusGProxy *pProxy, gboolean bSuspend, G_GNUC_UNUSED gpointer data)
{
	g_print ("Refresh dock after resuming: %d\n", bSuspend);
...
int main(int argc, char **argv)
{
	...
	if (pProxy == NULL)
	{
		g_print ("Login1 bus not available, can't connect to 'resuming' signal\n");
		return EXIT_FAILURE;
	}
	...
	dbus_g_proxy_add_signal (pProxy, "PrepareForSleep", G_TYPE_BOOLEAN,
		G_TYPE_INVALID);
	dbus_g_proxy_connect_signal (pProxy, "PrepareForSleep",
	...
	dbus_g_proxy_disconnect_signal (pProxy, "PrepareForSleep",
